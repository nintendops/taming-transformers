...
# DECODER MODEL
class StyleAE(pl.LightningModule):
    '''
    ...
    def __init__(self,
        ...
        c_dim = 16 * 16 * ddconfig["z_channels"]
        self.encoder_fc = StyleGAN.FullyConnectedLayer(c_dim, w_dim, activation='relu')
        ##########################################

        self.decoder = StyleGANDecoder(**ddconfig)
    ...
    def encode(self, x, mask):
        ...
        return h, ws, mask_out

    def decode(self, h, ws):
        h = self.post_bottleneck_conv(h)
        dec, _ = self.decoder(h, ws)
        return dec
    ...
    def forward(self, batch, quant=None, mask_in=None, mask_out=None, return_fstg=True, use_noise=True, debug=False):
        ...
        if quant is None:
            if use_noise and self.first_stage_model_type == 'vae':
